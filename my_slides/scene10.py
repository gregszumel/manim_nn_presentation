...
class Slide10RealWorld(Slide):
    def construct(self):
        ...
        title = section_title("the world is full of functions")
        self.play(FadeIn(title), run_time=0.5)

        # ── Layout constants ───────────────────────────────────────
        NET_X = 1.2  # x-center for all networks
        FLAT_X = -2.4  # x-center for vertical input columns
        GRID_X = -4.6  # x-center for grids / waveform / tokens
        ...
        cat_lbl = Text("cat image", font_size=16, color=C_GREY).next_to(
            cat_grid, DOWN, buff=0.12
        )
        ...
        self.play(
            *[
                cat_grid[i].animate.move_to(col1_pos[i]).scale(PIX_SCALE)
        ...
        out_cat = Text('"cat"', font_size=22, color=C_GREEN).next_to(
            net1, RIGHT, buff=0.3
        )
        ...
        dog_lbl = Text("dog image", font_size=16, color=C_GREY).next_to(
            dog_grid, DOWN, buff=0.12
        )
        out_notcat = Text('"not cat"', font_size=22, color=C_RED).next_to(
            net1, RIGHT, buff=0.3
        )
        ...
        self.play(
            *[
                dog_grid[i].animate.move_to(col1_pos[i]).scale(PIX_SCALE)
                for i in range(N_PIX)
            ],
            FadeOut(dog_lbl),
            run_time=0.8,
        ...
        waveform = ax_audio.plot(
            waveform_fn, x_range=[0, 4 * PI], color=C_INPUT, stroke_width=2
        )
        audio_lbl = Text("audio", font_size=16, color=C_GREY).next_to(
            ax_audio, DOWN, buff=0.1
        )
        ...
        h_nums = VGroup(
            *[
                DecimalNumber(
                    v, num_decimal_places=1, font_size=11, color=C_YELLOW
        ...
        out_audio = Text('"Ten-four"', font_size=22, color=C_GREEN).next_to(
            net2, RIGHT, buff=0.3
        )
        ...
        id_labels = VGroup(
            *[
                Text(str(tid), font_size=11, color=C_CLAIM).move_to(
                    token_rects[i].get_center()
                )
        ...
        out_llm = Text('"Ketchup"', font_size=22, color=C_GREEN).next_to(
            net3, RIGHT, buff=0.3
        )
